# What's in the folders above
AI projects implemented using Python ```3.10.6```. The class Artificial Intelligence (CSC-320) was taught by TJ Schlueter from Sep-Nov 2023 at [Union College](https://www.union.edu/). Some projects are self motivated ones as well which are inspired by CS50 AI course by Harvard.
# Have been uploaded 
1. ## Chatbot ELIZA
``` mermaid
flowchart TD
    A[User Input Text] --> B[Clean & Tokenize Input String]
    B --> C{Match Keywords/Patterns?}
    C -->|Yes| D[Select Highest Priority Rule]
    C -->|No| E[Apply Fallback Generic Prompt]
    D --> F[Perform Deictic Shift <br> e.g., swap 'my' to 'your']
    F --> G[Assemble Response Template]
    E --> H[Output Response to User]
    G --> H
    H --> A
end
```
2. Human and borg problem (very similar to the Missionaries and Cannibals
4. A* Informed search algorithm to solve a 9 cell puzzle where one cell is blank
5. Neural Networks
6. Network of actors which is an implementation of graph search to tell the number of actors connecting two actors.
7. Knight and Knaves a logic based game to deduce which character is a knight or a knave.
8. Tictactoe game playing against AI
9. Ultimate tictactoe
# Yet to be uploaded
Minimax and Genetic Algorithm projects yet to be posted...

# Contact
Email: mfarooqjuly16@gmail.com

